...
def read_log(fname, data_type):
    ...
    # find line in ASCII file where triaxial ppv data starts
    for lnx, lni in enumerate(range(1,100)):
        current_line = str(np.genfromtxt(workingDirectory+"\\"+fname, skip_header=lnx, max_rows=1, delimiter='	',dtype=data_type, autostrip=True))
        # print(current_line)
        # stop reading file if exceedance/summary file
        if (('Tran' in current_line) and ('Vert' in current_line)) and not ('Geo' in current_line):
            return None, None, None
        
        # assuming month precedes space
        space_pos = current_line.find(' ')
        segment = current_line[0:space_pos]
        if segment in ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', \
            'October', 'November', 'December']:
            dateLine = lni # line containing month - assumes data follows it
    ...
    for lnx, lni in enumerate(range(1,100)):
        current_line = str(np.genfromtxt(workingDirectory+"\\"+fname, dtype=data_type, skip_header=lnx, max_rows=1, autostrip=True))

        if (('Tran' in current_line) and ('Vert' in current_line)) and not ('Geo' in current_line):
            return None, None, None
        
        # find 'Serial Number' line
        if 'Serial' and 'Number' in current_line:
            locB = current_line.find('B') # locate 'B' to find the start of the minimate name
            ...
            if '"' in current_line:
                ...
                adjust = -2
            monitorName = current_line[locB:adjust]
            break
...
# The monitoring dates layer contains:
    ...
    def _getMonitoringDuration_(fname):
        ...
        # determine monitor name
        for lnx, lni in enumerate(range(1,100)):
            current_line = str(np.genfromtxt(workingDirectory+"\\"+fname, dtype=str, skip_header=lnx, max_rows=1))
            
            # find 'Serial Number' line
            if 'Number' and 'Intervals' in current_line:
                loc_colon = current_line.find(':') # locate 'B' to find the start of the minimate name
                
                # check to see if " is in num_intervals
                if '"' in current_line:
                    adjust = -3
                else:
                    adjust = -2
                num_intervals = float(current_line[loc_colon+4:adjust])
                
                interval_flag = True
            
            if 'Interval' and 'Size' in current_line:
                loc_colon = current_line.find(':') 
            
                numbers_in_line = [val for val in current_line if val.isnumeric()]
                
                data_rate = float(''.join([str(nums) for nums in numbers_in_line]))
                
                if 'seconds' not in current_line:
                    interval_size = float(data_rate)*60
                elif 'seconds' in current_line:
            ...
            if interval_flag & duration_flag:
                break
...
def calcPCPV():
    for monitorName in data_storage.keys():
        for day in data_storage[monitorName]:
            domFreq = []
            temp_PPV = data_storage[monitorName][day][0][:,[1,3,5]]
            
            # replace *** (overload) with a zero reading
            for lnx, lnv in enumerate(temp_PPV[:,0]):
                if (lnv == '***') or (lnv == 'nan'):
                    temp_PPV[lnx,0] = '0'
            for lnx, lnv in enumerate(temp_PPV[:,1]):
                if (lnv == '***') or (lnv == 'nan'):
                    temp_PPV[lnx,1] = '0' 
            for lnx, lnv in enumerate(temp_PPV[:,2]):
                if (lnv == '***') or (lnv == 'nan'):
                    temp_PPV[lnx,2] = '0'
            
            # convert to float for numerical comparison
            temp_PPV = temp_PPV.astype(float)
    ...
    for monitorName in data_storage.keys():
        for day in data_storage[monitorName]:
            for ppv_idx in range(0,len(data_storage[monitorName][day][0][:,8])):
                ...
                # check if ppv value read exceeds criterion
